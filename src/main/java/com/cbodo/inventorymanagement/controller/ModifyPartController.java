...
/**
 * ModifyPartController controls logic of modify part form.
 *
 * FUTURE ENHANCEMENT:
 ...
 * @author Craig Bodo
 */
public class ModifyPartController extends MainController {
    /**
     * Index of part in inventory
     */
    private int index;
    /**
    ...
    /**
     * Imports data from selected part into form fields.
     * @param selectedPart Part being modified.
     */
    public void initData(Part selectedPart) {
        ...
        if (selectedPart.getClass().getName().contains("Outsourced")) {
            Outsourced outsourcedPart = (Outsourced) selectedPart;
            inHouseOrOutsourcedGroup.selectToggle(this.outsourcedRadioButton);
            if (outsourcedPart.getCompanyName() == null || outsourcedPart.getCompanyName().isEmpty()) {
                outsourcedPart.setCompanyName("Sample Company");
            }
            inHouseOrOutsourcedTextField.setText(outsourcedPart.getCompanyName());
        }
        else if (selectedPart.getClass().getName().contains("InHouse")){
            InHouse inHousePart = (InHouse) selectedPart;
            inHouseOrOutsourcedGroup.selectToggle(this.inHouseRadioButton);
            if (inHousePart.getMachineId() == 0) {
                inHousePart.setMachineId(1);
            }
            inHouseOrOutsourcedTextField.setText(String.valueOf(inHousePart.getMachineId()));
        }
    }
